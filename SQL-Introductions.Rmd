---
title: "Data Management with SQL"
institute: "Woods Hole Oceanographic Institution"
date: "2021-06-02"
output:
  xaringan::moon_reader:
    lib_dir: libs
    nature:
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
---

# .right[Brett Longworth]

.pull-left[![Brett Longworth](images/brett.JPG)]

.pull-right[

Research Associate in Geology and Geophysics

**Research Interests:** Accelerator Mass Spectrometry, radiocarbon, data mining for method development, links between the marine and terrestrial carbon cycle.

**Favorite Database:** The NOSAMS DB. We have data on every sample run since 1992!

]

---

# .right[Joe Futrelle]

.pull-left[![Joe Futrelle](https://avatars2.githubusercontent.com/u/2365298?s=400&u=97004bc2d84a168bcf9b2fd81f03bcde7a15e166&v=4)]

.pull-right[


Software Engineer at WHOI

**Research Interests:** Open science, open data, databases, data-intensive computing, machine learning, software engineering best practices

**Favorite Database:** GitHub, essentially a dastabase for all the cool code in the world.

]

---

# .right[Audrey Mickle]

.pull-left[![Audrey Mickle](images/IMG_1384.JPG)]

.pull-right[


WHOI Data Librarian

**Research Interests:** Open data, open access, data management, and bibliometrics.



**Favorite Database:** Symplectic Elements, which collects, analyses, showcases grants and scholarly activities. 

]

---

# .right[Lyndsey Lefebvre]

.pull-left[![Lyndsey Lefebvre](images/lyndsey_lefebvre.jpg)]

.pull-right[


Research Associate in Biology

**Research Interests:** Reproductive physiology & ecology of fishes; data management fledgling but promoter of SQL.

**Favorite Database:** I’m not that involved with data management that I have a favorite, but I love me a clean, well documented relational database.

]

---

# .right[Kodiak Firesmith]

.pull-left[![Kodiak Firesmith](images/kodiak_firesmith.jpg)]

.pull-right[


Systems Administrator at WHOI

**Research Interests:** Resilient operations, effective Linux and FOSS software management, Ops best practices, container management.

**Favorite Database:** Postgresql, an open source, client-server relational database system.

]

---

## SQL Databases are everywhere!

![IFCB Dashboard](images/ifcb_dashboard.png)

---

# Course resources

## Website

https://blongworth.github.io/2021-06-02-WHOI-SQL/

## Slack

https://whoi-datascience.slack.com

https://join.slack.com/t/whoi-datascience/shared_invite/zt-fhox2nmo-6gi24Ddq4raKH2oocCNoWw

\#databases channel

---

# A note about survey.db

![wild data](images/wild_data.jpeg)

---

# Getting help and giving feedback

We are here to help! Raise your hand and a helper or instructor will message you in the chat. If it can't be solved via chat you and the helper will move to a breakout room to talk through the problem.

We'll also ask for input and feedback often. Use "yes" and "no" in the participants window. Coffee break indicator also welcome, but we've got a lot to cover!

![Zoom feedback tools](images/zoomtools.png)
---

# Code of Conduct

We are dedicated to providing a welcoming and supportive environment for all people, regardless of background or identity. Any form or behavior to exclude, intimidate, or cause discomfort is a violation of the Code of Conduct. In order to foster a positive and professional learning environment we encourage the following kinds of behavior in all platforms and events:

* Use welcoming and inclusive language
* Be respectful of different viewpoints and experiences
* Gracefully accept constructive criticism
* Focus on what is best for the community
* Show courtesy and respect towards other community members

---

class: center, middle

![Carpentries logo](https://carpentries.org/assets/img/TheCarpentries.svg)


![Software carpentry](images/swc_logo.png)

.footnote[The Carpentries: https://carpentries.org/]

---

class: center, middle

# Thanks!

Slides created via the R package [**xaringan**](https://github.com/yihui/xaringan).

The chakra comes from [remark.js](https://remarkjs.com), [**knitr**](http://yihui.org/knitr), and [R Markdown](https://rmarkdown.rstudio.com).

